...
[source,shell]
----
az keyvault create --name kvquarkusazurekeyvault0423 \
...
Then, assign all secret permissions(backup, delete, get, list, purge, recover, restore, set) to yourself with (change `<subscription>` with your subscription id):

[source,shell]
----
az ad signed-in-user show --query id -o tsv | az keyvault set-policy --name myKeyVault --object-id @- --secret-permissions all
----
...
[source,shell]
----
az keyvault show --name kvquarkusazurekeyvault0423\
    --resource-group rg-quarkus-azure-keyvault\
...
[source,properties]
----
quarkus.azure.keyvault.secret.endpoint=https://kvquarkusazurekeyvault0423.vault.azure.net/
----
